...
task_history = utils_11.get_task_history()  # Fetch past tasks from database
...
admin_usernames = utils_11.get_admin_usernames()
...
with st.form("task_creation_form"):
    ...
    if submit:
        if title and description and assigned_to:
            ...
            task_id = utils_11.create_task(title, description, assigned_to, urgency)
...
tasks = utils_11.get_open_tasks()  # Retrieve all open tasks
...
for task in tasks:
    ...
    if st.session_state.username in task['assigned_to']:
        if st.button("Gördüm", key=f"seen_{task['task_id']}"):
            utils_11.mark_task_as_seen(task['task_id'], st.session_state.username)
            st.rerun()
        ...
        if task['status'] == 'in progress':
            ...
            if st.button("İlerlemeyi Güncelle", key=f"update_progress_{task['task_id']}"):
                utils_11.update_task_progress(task['task_id'], progress)
                st.success("Görev ilerleme durumu güncellendi.")
                st.rerun()

        if st.button("İşi Kapat", key=f"complete_{task['task_id']}"):
            utils_11.complete_task(task['task_id'])
            st.rerun()

# Completed tasks section
st.subheader("Tamamlanmış Görevler")
completed_tasks = utils_11.get_completed_tasks()  # Retrieve completed tasks
